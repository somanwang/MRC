CREATE OR REPLACE PACKAGE hw_conc_utl AS
  /*==================================================
  DESCRIPTION:
       This program provide common API for concurrent procedure
   HISTORY:
     1.00   2005-09-26   jim.lin   Creation
  ...
  * =============================================*/
  ...
  PROCEDURE out_message_list;

END hw_conc_utl;
/
CREATE OR REPLACE PACKAGE BODY hw_conc_utl AS
  /*==================================================
  ...
  BEGIN
    ...
    log_msg('----------------------------------------');
    FOR c IN c_conc(l_request_id)
    LOOP
      log_msg(rpad(c.end_user_column_name,
                   30) || ' : ' || l_arg_val_tbl(i));
      i := i + 1;
    END LOOP;
